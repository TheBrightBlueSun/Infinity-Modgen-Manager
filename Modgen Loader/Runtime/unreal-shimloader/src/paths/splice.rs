...
mod tests {
    ...
    fn test_splice_path_exact_match_no_trailing_separator() {
        ...
        assert!(
            !bytes.ends_with('\\'),
            "remap target must not end with backslash, got {bytes:?}"
        );

        assert_eq!(
            bytes,
            "D:\\overlay\\Wrap\\UE4SS-settings.ini"
        );
    }
    ...
    fn test_dll_path_remapping() {
        let path = NormalizedPath::new(
            r"D:\Games\steamapps\common\ASTRONEER\Astro\Binaries\Win64\Mods\AutoIntegrator\dlls\main.dll"
        );

        let source = NormalizedPath::new(
            r"D:\Games\steamapps\common\ASTRONEER\Astro\Binaries\Win64\Mods"
        );

        let target = NormalizedPath::new(
            r"C:\Users\Test\Mods"
        );
        ...
        assert_eq!(
            result,
            Some(PathBuf::from(
                r"C:\Users\Test\Mods\autointegrator\dlls\main.dll"
            ))
        );
    }
    ...
    fn test_exe_dir_join_mods() {
        ...
        let exe_dir = PathBuf::from(
            r"D:\Games\steamapps\common\ASTRONEER\Astro\Binaries\Win64"
        );
    ...
    fn test_trailing_separator_mismatch() {
        ...
        // Use a normal escaped string for the trailing separator case.
        let source_with_slash = NormalizedPath::new(
            "D:\\Games\\Mods\\"
        );

        let source_without = NormalizedPath::new(
            r"D:\Games\Mods"
        );
        ...
        println!(
            ...
            source_with_slash.inner()
        );

        println!(
            "Source without trailing: {:?}",
            source_without.inner()
        );

        let result1 = splice_path(
            &path,
            &source_with_slash,
            &target
        );

        let result2 = splice_path(
            &path,
            &source_without,
            &target
        );
        ...
        println!(
            "Result with trailing slash source: {:?}",
            result1
        );

        println!(
            "Result without trailing slash source: {:?}",
            result2
        );
    }
    ...
    fn test_lib_rs_registration_scenario() {
        ...
        let dll_path = NormalizedPath::new(
            r"D:\Games\steamapps\common\ASTRONEER\Astro\Binaries\Win64\Mods\AutoIntegrator\dlls\main.dll"
        );
        ...
        assert!(
            result.is_some(),
            "DLL path should be remapped! Got: {:?}",
            result
        );
